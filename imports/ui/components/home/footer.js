Template.footer.helpers({
  languageSwitchers: function() {
    ...
    return Object.keys(supportedLanguages).map(function(languageCode) {
      return {
...
Template.footer.events({
  "click [data-lang]": function(event, template) {
    ...
    if (languageCode === TAPi18n.getLanguage()) return;

    TAPi18n.setLanguage(languageCode)
      .done(() => {
        Bert.alert(
          TAPi18n.__("alert_language_change", `${languageName} (${languageCode})`),
          "success",
          "growl-top-right"
        );
      })
      .fail(error => {
        Bert.alert(
          TAPi18n.__("alert_language_change_fail", `${languageName} (${languageCode})`),
          "danger",
          "growl-top-right"
        );
        console.log(error);
