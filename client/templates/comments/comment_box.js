...
Template.commentBox.helpers({
  discussion : function(){
    ...
    var sortComments = function(comments, parent){
        var cmnts = []
        comments
        .filter(function(comment) {   return comment.parent_id === parent })
  ...
  'click .upVote': function(event, template) {
    ...
    if (userId && !_.includes(this.upvotes, userId)) {
      ...
      if (userId && _.includes(this.downvotes, userId)) {
        ...
        Meteor.call("upVoteComment", commentId, function(error, result){ if(error){console.log("error", error);} });
  ...
  'click .downVote': function(event, template) {
    ...
    if (userId && !_.includes(this.downvotes, userId)) {
      ...
      if (userId && _.includes(this.upvotes, userId)) {
        ...
        Meteor.call("downVoteComment", commentId, function(error, result){ if(error){ console.log("error", error); }});
  ...
  'click .replyToComment': function(event, template) {
    ...
    if(_.includes(replyToComment, commentId)){
       ...
       Session.set("replyToComment",replyToComment);
        ...
        var replyForm = '<form class="addNewReply"> <textarea id="commentReply" class="form-control" rows="3" cols="30"></textarea><br><button type="submit" class="btn btn-default pull-right"> Reply</button></form>';
    ...
    if (!Meteor.userId()) {
      ...
      event.preventDefault();
       ...
       var comment = {
          ...
          authorName : Meteor.user().username,
          authorAvatar: Meteor.user().profile.avatar.default,
          text : $('#commentReply').val().replace(/\r?\n/g, '<br />'),
          parent_id : commentId
       }
       var replyToComment = Session.get("replyToComment");
       replyToComment = _.pull(replyToComment, commentId);
