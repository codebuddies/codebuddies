...
Meteor.users.search = function(query) {
  var cleanQueryString = RegExp.escape(query);
  var insensitiveCleanQueryString = new RegExp(cleanQueryString, "i");
  return Meteor.users.find({$or: [ {'user_info.profile.email':{ $regex:insensitiveCleanQueryString}},{'user_info.name':{ $regex:insensitiveCleanQueryString}},{'user_info.real_name':{ $regex:insensitiveCleanQueryString}} ] });
};
