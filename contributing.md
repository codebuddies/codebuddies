...
6. Type `meteor npm install`, and then `meteor --settings settings-development.json` in your terminal to start up the app in your browser ([http://localhost:3000](http://localhost:3000)).
  ...
  * You can also run the app locally in a Docker container (this is still in the early stages of development). To do so, first [download and install Docker](https://www.docker.com). After it's install, run Docker, then in your terminal in the codebuddies directory, first type `docker build . -t codebuddies`. After the container builds, type `docker run -i -t -p 3000:3000 codebuddies`, then you should be able to view the app at ([http://localhost:3000](http://localhost:3000)).
